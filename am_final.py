...
ReferenceImages = ["arrowl_final.jpg","arrowr_final.jpg","arrowstop_final.jpg","arrowt_final.jpg","go_final.jpg"]
ReferenceTitles=["left","right","stop","turn","go"]
...
def resize_and_threshold_warped(image):
    ...
    warped_new_gray = cv2.cvtColor(warped_new, cv2.COLOR_BGR2GRAY)
        ...
        for cnt in contours:
            ...
            if len(approx)==4:
                ...
                if area > minSquareArea:
                    ...
                    print(len(warped_eq))
                    for i in range(6):
                        diffImg = cv2.bitwise_xor(symbol[i].img,warped_eq)
                        print(diffImg)
